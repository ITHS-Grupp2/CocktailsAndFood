...
export function MatchDrinkService(mealId: string) {
  const recommendedDrink: Drink = {
    drinkId: matchData.drinkMatch.find((d) => d.burgerId === mealId)
      ?.drinkId as string,
    percentage: matchData.drinkMatch.find((d) => d.burgerId === mealId)
      ?.percentage as number,
    motivation: matchData.drinkMatch.find((d) => d.burgerId === mealId)
